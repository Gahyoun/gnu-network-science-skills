...
- **짧은 L 사다리에서 ν 과소추정.** L_max/L_min ≲ 4이면 correction-to-scaling을 흡수하지
  못해 ν가 계통적으로 10% 가까이 낮게 나온다(2D Ising 40000-sweep 데이터에서 ν = 0.90~0.94로
  재현). 사다리를 8배 이상 벌리거나, 그럴 수 없으면 ν를 점추정이 아니라 **하한**으로 보고하고
  프로토콜(fit window, L_min, 다항식 차수)을 바꿔 가며 나온 폭을 계통오차로 함께 낸다.
